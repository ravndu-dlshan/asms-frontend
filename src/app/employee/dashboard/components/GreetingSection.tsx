...
export default function GreetingSection({ userImage }: GreetingSectionProps) {
  ...
  useEffect(() => {
    ...
    if (user?.firstName) {
      ...
      setFullName(user.lastName ? `${user.firstName} ${user.lastName}` : user.firstName);
    }
  }, []);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-5 items-start w-full mb-5">
      {/* Greeting Card + Stats */}
      ...
      <div className="w-full lg:flex-1 relative">
        <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-5 border border-gray-700 flex flex-col justify-between relative">
          <div className="flex space-x-6 px-6 relative">
            <Link href="/employee/profile">
              <div className="flex items-center space-x-3">
                ...
                <div className="text-white">
                  <p className="font-semibold text-sm sm:text-base">{fullName}</p>
